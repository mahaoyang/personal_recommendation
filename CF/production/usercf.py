...
def debug_duplicate(user_click, recom_result):
    intersection = []
    for i in user_click:
        intersection.append('%s:%s' % (i, ','.join(list(set(user_click[i]) & set(recom_result[i].keys())))))
    return '\n'.join(intersection)
...
def main_flow():
    ...
    print(debug_duplicate(user_click, recom_result))
